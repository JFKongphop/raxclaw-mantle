/**
 * raxclaw — RAXC  |  Autonomous Security Cognition on Mantle
 *
 * Usage:
    ...
    <Box flexDirection="column" paddingX={1}>
      ...
      >
        ...
        <Text>
          ...
          {"  Analyze a contract (direct RAXC audit)"}
        </Text>
        <Text>
          {"  "}
          <Text color="green" bold>
...
const AnalyzeUI: FC<{ contract: string }> = ({ contract }) => {
  ...
  useEffect(() => {
    ...
    if (contract.endsWith(".sol")) env.RAXC_CONTRACT_FILE = path.resolve(contract);
    else env.RAXC_CONTRACT_CODE = contract;
  ...
  return (
    <Box flexDirection="column">
      ...
      {!done && (
        <Text color="green">
          <Spinner type="dots" />
          <Text dimColor>{"  Running RAXC audit..."}</Text>
        </Text>
      )}
      {done && (
        <Text bold color={code === 0 ? "green" : "red"}>
...
function findReports(): ReportMeta[] {
  const dir = path.join(REPO_ROOT, "backend-typescript", "reports");
